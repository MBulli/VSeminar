...
FeedbackFMGenerator::FeedbackFMGenerator(float carrier, float modulation, float modIndex, Envelope* env)
	: phaseCarrier(0),
	phaseModulation(0),
	freqCarrier(carrier),
...
FeedbackFMGenerator::~FeedbackFMGenerator()
{
}


float FeedbackFMGenerator::process(double sampleRate, long long currentTime)
{
	...
	float y = sin(phaseCarrier + modulationIndex * previousValue);
...
void FeedbackFMGenerator::reset()
{
	phaseCarrier = 0;
	phaseModulation = 0;
}