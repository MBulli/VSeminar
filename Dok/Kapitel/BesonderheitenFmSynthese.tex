...
\begin{equation*}
\theta(t)=\int_0^t{\omega_m(t)} dt = \int_0^t{\omega_0 + k_{FM}\cdot f(t)} dt = \omega_0t + k_{FM} \cdot \int_0^t{f(t)} dt
\end{equation*}
Setzt man diesen Term in \(s(t)\) ein, erhält man die Formel für ein frequenzmoduliertes Signal
\begin{equation}
s_{FM}(t)=A\cdot sin(\omega_0t + k_{FM} \cdot \int_0^t{f(t)} dt)
\label{eq:s_fm}
\end{equation}
...
\begin{equation}
k_{PM}\cdot p(t)=k_{FM} \cdot  \int_0^t{f(t)} dt
\end{equation}
...
s_{FM}(t)&=&A\cdot sin(\omega_0t+k_{FM}\cdot \int_0^t{sin(\omega_m t)} dt)
\end{eqnarray*}
...
% TODO formatierung
\begin{equation*}
k=N_1+n\cdot N_2 \mbox{fuer} n=...,-2,-,1,0,1,2,...
\end{equation*}
